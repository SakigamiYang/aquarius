...
package object codec {
  ...
  /**
   * A utility for encoding strings and byte arrays to a MIME base64 string,
   * and decoding from strings encoded in MIME base64 to byte arrays.
   *
  ...
  trait Base64StringEncoder extends StringEncoder {
    ...
    override def encode(bytes: Array[Byte]): String =
      new String(encoder.encode(bytes), StandardCharsets.UTF_8)

    override def decode(str: String): Array[Byte] = decoder.decode(str.getBytes(StandardCharsets.UTF_8))
  }
   ...
   *
   * This trait is thread-safe because there are no streams shared outside of method scope,
   * and therefore no contention for shared byte arrays.
   *
   * The encoding for strings is UTF-8.
   *
   * Gzipping inherently includes base64 encoding
   * (the GZIP utilities from java will complain otherwise!)
   */
